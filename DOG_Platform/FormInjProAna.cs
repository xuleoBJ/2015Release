...
namespace DOGPlatform
{
    public partial class FormInjProAna : Form
    {
        ...
        private void btnCalResult_Click(object sender, EventArgs e)
        {
            ...
            List<ItemDicLayerData> listDicLayer = cIODicLayerData.readDicLayerData2struct();
            ItemDicLayerData wellInjectLayerdata=listDicLayer.Find(p=>p.sJH==this.cbbWellInject.SelectedItem.ToString()
                && p.sXCM==sSelectedLayer);
            for (int i = 0; i < dgvInj2Pro.RowCount-1; i++) 
            {
                string sJHProdcut = dgvInj2Pro.Rows[i].Cells[1].Value.ToString();
                  ItemDicLayerData wellProductLayerdata=listDicLayer.Find(p=>p.sJH==sJHProdcut
                && p.sXCM==sSelectedLayer);
                //计算井距
