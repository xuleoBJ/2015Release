...
namespace DOGPlatform
{
  ...
  //          cCalConnect.generateConnectFile(ltStrJHwater, ltStrJHoil, ltStrSelectedLayer[0], sSelectedYYYYMM, fileDrawMapSourceConnect);
        ...
        void generateWellLayerPositonDrawData()
        {
            ...
            if (File.Exists(cProjectManager.filePathWellTypeDic) && File.Exists(cProjectManager.filePathLayerDataDic))  //进入生产状态，计算了井型代码
            {
        ...
        {
            if (tabControlLayerMap.SelectedIndex == 1 && File.Exists(cProjectManager.xmlConfigLayerMap) == false)
        ...
        private void btnAddDynamicData_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(fileNameWaterInject))
            {
                ...
                List<wellLayerValue> listWellDynamicValue = new List<wellLayerValue>();
